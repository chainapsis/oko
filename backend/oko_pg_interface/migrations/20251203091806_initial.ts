...
export async function up(knex: Knex): Promise<void> {
  ...
  // public.admin_users
  //
  await knex.schema.withSchema("public").createTable("admin_users", (table) => {
    table
      .uuid("user_id")
      .notNullable()
  ...
  await knex.schema
    ...
    .createTable("customer_dashboard_users", (table) => {
      ...
      table.index(["customer_id"], "idx_customer_dashboard_users_customer_id");
    });
  ...
  // public.tss_stages
  //
  await knex.schema.withSchema("public").createTable("tss_stages", (table) => {
    table
      .uuid("stage_id")
      .notNullable()
      .defaultTo(knex.raw("gen_random_uuid()"))
  ...
  await knex.raw(`
    CREATE INDEX idx_ks_node_health_checks_node_id_created_at
    ON public.ks_node_health_checks (node_id, created_at DESC)
  `);

  await knex.raw(`
    CREATE INDEX idx_server_keypairs_is_active
