...
CREATE UNIQUE INDEX key_share_nodes_server_url_uniq_active ON public.key_share_nodes USING btree (server_url) WHERE (deleted_at IS NULL);
...
CREATE INDEX idx_ks_node_health_checks_node_id_created_at ON public.ks_node_health_checks USING btree (node_id, created_at DESC);


-- public.ks_node_logs definition

-- Drop table

-- DROP TABLE public.ks_node_logs;

CREATE TABLE public.ks_node_logs (
...
CREATE UNIQUE INDEX idx_referrals_unique_source ON public.referrals USING btree (public_key, origin, COALESCE(utm_source, ''), COALESCE(utm_campaign, ''));
