...
export async function signInEd25519(
  db: Pool,
  email: string,
  jwt_config: {
    secret: string;
  ...
  try {
    ...
    return {
      success: false,
      code: "UNKNOWN_ERROR",
      msg: `signInEd25519 error: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
}

export interface CheckEd25519WalletResponse {
  has_wallet: boolean;
  wallet_id?: string;
  public_key?: string;
}

export async function checkEd25519Wallet(
  db: Pool,
  email: string,
): Promise<OkoApiResponse<CheckEd25519WalletResponse>> {
  try {
    const getUserRes = await getUserByEmail(db, email);
    if (getUserRes.success === false) {
      return {
    ...
    if (getUserRes.data === null) {
      // User doesn't exist - no ed25519 wallet
      return {
        success: true,
    ...
    if (walletRes.data === null) {
      // User exists but no ed25519 wallet
      return {
        success: true,
        data: { has_wallet: false },
      };
    ...
    return {
      success: false,
      code: "UNKNOWN_ERROR",
      msg: `checkEd25519Wallet error: ${error instanceof Error ? error.message : String(error)}`,
    };
  }
}
