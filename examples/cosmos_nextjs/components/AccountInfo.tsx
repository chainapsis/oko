...
export default function AccountInfo({ className }: AccountInfoProps) {
  ...
  const { data: balance } = useQuery({
    ...
    queryFn: async () => {
      if (!bech32Address) {
        throw new Error("Bech32 address is not available");
      }

      const client = await StargateClient.connect(chainInfo.rpc);
      const balance = await client.getBalance(bech32Address, "uosmo");

      const formatted = new CoinPretty(chainInfo.currencies[0], balance.amount);
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-col gap-6">
        <CopyableAddress value={bech32Address!} format={formatAddress} />
