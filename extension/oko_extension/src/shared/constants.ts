// oko_attached endpoints
export const OKO_ATTACHED_URL =
  import.meta.env.VITE_OKO_SDK_ENDPOINT || "https://attached.oko.app";

// API key for SDK initialization
export const OKO_API_KEY = import.meta.env.VITE_OKO_API_KEY || "";

// Extension message targets
export const MESSAGE_TARGET = {
  CONTENT: "oko-content",
  BACKGROUND: "oko-background",
  INJECTED: "oko-injected",
} as const;

// Wallet name for wallet-standard
export const OKO_WALLET_NAME = "Oko" as const;

// Icon data URI (from oko_sdk_svm)
export const OKO_ICON =
  "data:image/png;base64,";
